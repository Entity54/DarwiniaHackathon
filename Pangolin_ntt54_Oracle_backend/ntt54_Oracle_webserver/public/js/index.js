...
socket.on('connect', function () {
  ...
  socket.on('response_erc20_getBalanceBtn', function (msg) {
    getBalance_lbl.innerHTML = msg.balance;
  });
  ...
  const updateQuotesTable = function (quotesObjArray) {
    jQuery('#quotesTable').html('');
    quotesObjArray.forEach((quoteObj) => {
      let rowTemplate = jQuery('#quotesTemplate').html();
      let rowTemplatehtml = Mustache.render(rowTemplate, quoteObj);
      jQuery('#quotesTable').append(rowTemplatehtml);
    });
  };
